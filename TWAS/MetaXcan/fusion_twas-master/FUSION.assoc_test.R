...
idx <- which(!is.na(qc$flip) & qc$flip)
sumstat$Z[idx] = -1 * sumstat$Z[idx]
sumstat$A_EFFECT[idx] = genos$bim[idx, 5]
sumstat$A_ALT[idx] = genos$bim[idx, 6]


# Remove strand ambiguous SNPs (if any)
if (sum(!qc$keep, na.rm = TRUE) > 0) {
