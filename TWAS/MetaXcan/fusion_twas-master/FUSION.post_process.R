...
# Función para extraer el nombre del panel desde la ruta en la columna FILE
extract_panel <- function(file_path) {
  if (is.na(file_path) || file_path == "") return(NA)
  parts <- strsplit(file_path, "/")[[1]]
  if(length(parts) >= 2) {
    return(parts[length(parts)-1])  # El penúltimo folder como nombre del panel
  } else {
    return(NA)
  }
}

# Rellenar PANEL si está vacío o NA
if (!"PANEL" %in% colnames(wgtlist)) {
  wgtlist$PANEL <- NA
}
wgtlist$PANEL <- ifelse(is.na(wgtlist$PANEL) | wgtlist$PANEL == "",
                        sapply(wgtlist$FILE, extract_panel),
                        wgtlist$PANEL)


# --- FILTRAR ARCHIVOS INVÁLIDOS EN wgtlist$FILE ---
wgtlist$FILE <- as.character(wgtlist$FILE)

valid_files <- !is.na(wgtlist$FILE) & wgtlist$FILE != "" & file.exists(wgtlist$FILE)

if (sum(valid_files) == 0) {
  stop("ERROR: No se encontraron archivos válidos en wgtlist$FILE")
} else {
  cat("Archivos válidos encontrados:", sum(valid_files), "de", nrow(wgtlist), "\n")
}

wgtlist <- wgtlist[valid_files, ]
...
if ( opt$omnibus ) {
  ...
  for ( g in dup.genes ) {
    ...
    for ( i in 1:length( cur.keep ) ) {
      this_file = wgtlist$FILE[ cur.keep[i] ]
      # Check for invalid file strings before file.exists
      if (length(this_file) == 0 || is.null(this_file) || is.na(this_file) || this_file == "") {
        cat("WARNING: Archivo no válido (NA, NULL, vacío o longitud 0): ", this_file, "\n", file=stderr())
        cur.drop[i] = TRUE
        next
      }
      if (!file.exists(this_file)) {
        cat("WARNING: Archivo no encontrado: ", this_file, "\n", file=stderr())
        cur.drop[i] = TRUE
        next
      }
      cat("Cargando archivo: ", this_file, "\n")
      load(this_file)
      ...
      if ( length(mod) == 0 ) cur.drop[i] = TRUE
      ...
      if ( !is.na(opt$omnibus_corr) ) {
        ...
        for ( i in order(ge_z^2,decreasing=T) ) {
          if ( !pruned[i] ) {
            ...
            pruned[ ge_g.cor[ i , ]^2 > opt$max_r2 ] = TRUE
            pruned[i] = FALSE
          }
        }
        ...
        if ( sum(!pruned) > 1 ) {
          ...
          pv.chi = pchisq( chisq , df=sum(!pruned) , lower.tail=FALSE)
          cur.tbl = ge_g.cor[!pruned,!pruned]
          cur.tbl[lower.tri(cur.tbl,diag=TRUE)] = NA
          cur.tbl = as.data.frame(as.table(cur.tbl))
          cur.tbl = cur.tbl[!is.na(cur.tbl[,3]),]
          write.table( cbind(g,format(cur.tbl,digits=3)) , quote =FALSE , col.names=FALSE , row.names=FALSE , sep='\t' , file=paste( opt$out , ".CORR.dat" , sep='') , append=TRUE )
          cat( g , M , min( cur.wgt$TWAS.P ) , sum(!pruned) , pv.chi , '\n' , sep='\t' , file=paste(opt$out , ".omnibus.pv" , sep='') , append=TRUE )
        }
      }
    }
  }
...
genos.keep = rep(FALSE,nrow(genos$bim))
...
for ( i in 1:nrow(wgtlist) ) {
  this_file = wgtlist$FILE[i]
  print(paste("Chequeando archivo:", i, this_file))
  print(str(this_file))
...
qc <- allele.qc( sumstat$A_EFFECT , sumstat$A_ALT , genos$bim[,5] , genos$bim[,6] )

valid_flips <- which(!is.na(qc$flip) & qc$flip)

if (length(valid_flips) > 0) {
  sumstat$Z[valid_flips] <- -1 * sumstat$Z[valid_flips]
  sumstat$A1[valid_flips] <- genos$bim[valid_flips, 5]
  sumstat$A2[valid_flips] <- genos$bim[valid_flips, 6]
}
